...
public class Action {
    ...
    public String showall(){
    	String sq  = "select AuthorID from Book";
    	List<String> listauthorid = new LinkedList<String>();
    	List<String> listname = new LinkedList<String>();
    	DBConnection connect = new DBConnection();
    	List<String> listall = new LinkedList<String>();
    	listauthorid=connect.select(sq);//从书表中获得所有的作者id
        listall=connect.select("select * from Book");
        for (int i = 0; i < listauthorid.size(); i++){
        	String sql = "select Name from Author where authorid=?";
        	Matcher m1 = p.matcher(sql);
            String sql1 = m1.replaceFirst('"'+listauthorid.get(i)+'"');
            listname=connect.select(sql1);//获得与id相对应的name
            for (int j = 2; j<listall.size(); j++){
            	if (listall.get(j).equals(listauthorid.get(i))){
            		listall.set(j, listname.get(0));
            	}
            }
        }
    ...
    public String showauthor(){
    	...
    	DBConnection connect = new DBConnection();
    	List<String> listauthor = new LinkedList<String>();
    	listauthor=connect.select("select * from Author");
